...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    last_name = models.CharField(
        verbose_name="last name", max_length=30, null=True, blank=True
    )
    username = models.CharField(
        verbose_name="Username", max_length=30, unique=True, blank=True, null=True
    )
    email = models.EmailField(
        verbose_name="email", max_length=255, unique=True, blank=True, null=True
    )
...
class Otp(AuditFields):
    ...
    phone_number = models.CharField(
        ("phone number"), max_length=50, null=True, blank=True
    )
    ...
    def is_otp_valid(self):
        ...
        self.save()
        if (
            self.created_date + relativedelta(seconds=settings.OTP_VALID_TILL)
            > timezone.now()
            and self.attempts < 6
        ):
            return True
